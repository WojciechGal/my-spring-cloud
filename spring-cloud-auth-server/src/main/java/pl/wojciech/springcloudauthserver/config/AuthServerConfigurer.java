package pl.wojciech.springcloudauthserver.config;
...
public class AuthServerConfigurer extends AuthorizationServerConfigurerAdapter {
    ...
    public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
        clients.inMemory()
                .withClient("auth-server")
                .secret(passwordEncoder.encode("pass16"))
                .redirectUris("http://localhost:8182/login")
                .authorizedGrantTypes("authorization_code", "refresh_token")
                .scopes("auth-server-scope")
                .autoApprove(true)
